...
**Assignment submission (Sofia Rodas):** ______________________________________
...
ggplot() +
    ...
    labs(x = "Count",
         y = "Site",
         title = "California Spiny Lobster Count by Site") + 
    scale_fill_manual(values = c("red", "skyblue", "hotpink", "forestgreen", "orange"))  
   
    
```
The density ridge plot shows that the data from all of the sites are positively skewed data. In other words there are outliers in the data with counts reaching into the hundreds in the sites. Mohawk has a number of observations between the counts of 100 and 200. Isla Vista counts taper off more slowly than the other sited with a greater conglomeration of counts between 50 and 100. Isla Vista counts also have some observations between 150 and 300. Carpenteria also has a few peaks between 75 and 125, between 150 and 200, and between 240 and 275. Since the data is skewed right, the descriptive statistic of choice is the median. It reduces the influence of the outliers such as would be present if the mean was the statistic shown. 
...
```
The boxplot and beeswarm figure above shows that lobster counts increase as more time passes since the creation of MPAs. This figure shows that lobster populations increase fairly steadily from 2012 to 2018. 2016 is an outlier year that has fewer lobsters than 2015 as shown by the boxplot. Though more analysis is necessary, the creation of MPAs appears to positively impact lobster populations. 
...
```
The histogram above shows that MPA sites have a larger mean lobster size than non-MPA sites. The histogram also shows that there are more lobsters counted in MPA sites than non-MPA sites. 
...
```
There are on average approximately `r lobster_count` in non-MPA sites according to the Poisson regression model. There is an approximate `r perc_lob_inc` increase in the percentage of lobsters in MPA sites on average. 
...
check_model(m2_pois)
```
The posterior predictive check, the misspecified dispersion and zero-inflation check, and the distribution of quantile residuals do show that the observed data and the model-predicted data do not follow the same trends. The model does not accurately fit the data for these assumptions. The homogeneity of variance and influential observations are modeled well by the Poisson regression.

```{r}
check_overdispersion(m2_pois)
```
The dispersion ratio is 67.033 which is an extremely large value. The dispersion ratio should be equal to 1 which would indicate that the variance is equal to the mean. The observed data is over dispersed when utilizing this model. The over dispersion of the data indicates that a Poisson regrssion model is not a good fit for the data. 

```{r}
check_zeroinflation(m2_pois)
```
The observed zeros in this data are 27. This indicates that there are numerous zeros in the data. The Poisson model does not handle zeros well. The high number of zeros can be causing the overdispersion as well. 
...
```
There are on average approximately `r nb_lobster_count` in non-MPA sites according to the Poisson regression model. There is an approximate `r nb_perc_lob_inc` increase in the percentage of lobsters in MPA sites on average.

```{r}
check_overdispersion(m3_nb)
```

The over dispersion test is less close to 1. It is less than 5 and therefore within the range of good dispersion for the negative binomial model.

```{r}
check_zeroinflation(m3_nb)
```
The observed number of zeros is close to the predicted number of zeros meaning that the model accurately accounts for the zeros in its statistical analysis. The ratio is also near 1 indicating that the observed and predicted zero-inflations are close. 

```{r}
check_predictions(m3_nb)
```
The observed and predicted data follow the same trends in the posterior predictive check. The model accurately fits the data and shows the statistical trends. 

```{r}
check_model(m3_nb)
```
The homogeneity of variance, the influential observations, and the distribution of quantile residuals show the same trends between the observed and model-predicted data. The misspecified dispersion and zero-inflation assumption is not perfectly aligned between the predicted and observed data. The predicted data is within the confidence interval of the observed misspecified dispersion and zero-inflation data but does not perfectly follow the same values. This is an improvement from the Poisson model assumption checks.

The negative binomial GLM model is a better fit for the data than the Poisson regression model in this case because of the large amount of zeros in the data. Negative binomial models account for over dispersion by adding an additional parameter. The Poisson model was utilized as an investigative model first since it is less complex.
...
mix_mpa_treat <- exp(m5_fixedeffs$coef[2])

# calculate percent change of treatment as an MPA site
mix_perc_lob_inc <- round((mix_mpa_treat -1) *100, 2)

```


The intercept shows the average number of lobsters within non-MPA sites across all of the years. The treat variable shows the average percent change in lobsters for MPA sites. The individual years show the average percent change in lobsters for any given year for non-MPA sites. The treat and year interaction variables show the average percent change in lobsters for any given year for MPA sites.

The main effect of treat is negative because the values date back to the creation of the MPAs. MPAs are protection areas created when ecosystems are in decline. The MPAs were likely created where the lobster populations were the lowest hence the low values are distorting the true effects of MPA site creation. 
