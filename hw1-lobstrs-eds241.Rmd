...
ggplot() +
    ...
    labs(x = "Count",
         y = "Site",
         title = "California Spiny Lobster (*Panulirus Interruptus*) Count by Site") + 
...
ggplot() +
    ...
    labs(x = "MPA",
         y = "Count",
         title = "California Spiny Lobster (*Panulirus Interruptus*) Count by MPA Status") +
...
ggplot() +
    ...
    labs(x = "Count",
         y = "Site",
         title = "California Spiny Lobster (*Panulirus Interruptus*) Count by Year") +      scale_color_manual(values = c("#640D14", "skyblue", "hotpink", "forestgreen", "#D77A61", "navy", "#F4F1BB"))
...
spiny_counts |> 
#    filter(mean_size != "NaN") |> 
    group_by(mpa) |> 
    ggplot(aes(x = mean_size)) +
    geom_histogram(aes(fill = mpa)) +
    geom_vline(xintercept =  70) +
#     geom_vline(mapping = aes(xintercept = mean(mean_size))) +
#   facet_wrap(~mpa) +
    scale_fill_manual(values = c("navy", "hotpink")) +
    # geom_vline(data = mean_size,
    #            aes(xintercept = mean_mean_size), size=1) +
    labs(x = "Count",
         y = "Mean Size", 
...
```
According to the OLS model, on average there are 22.73 lobsters in non-MPA sites. MPA sites have on average 5.36 more lobsters than non-MPA sites. 
...
```
The two plots above show that the residuals of the OLS model run on the lobster data does not follow what is expected for an OLS model that fits the data well. In other words a different model should be used for this data because there is a high dispersion of the residuals. 
...
```
The blue line is the simulation of the model while the green line is the observed data. If the OLM linear model were a good fit for the the data the oberserved data line and the model-predicted data lines would follow a similar trend. The model check again shows that the OLM linear model is not the appropriate model for this data. 
