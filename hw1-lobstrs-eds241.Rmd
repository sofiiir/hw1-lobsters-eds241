...
spiny_counts <- spiny_counts |> 
    ...
    summarize(counts = sum(count, na.rm = TRUE), mean_size = mean(size_mm, na.rm = TRUE)) |> 
    ungroup()
...
spiny_counts |> 
    group_by(site) |> 
ggplot() +
    geom_density_ridges(aes(x = counts, y = site, color = site),
                        quantile_lines=TRUE,
                        quantile_fun = median) +
    labs(x = "Count",
         y = "Site",
         title = "California Spiny Lobster (*Panulirus Interruptus*) Count by Site") + 
    scale_color_manual(values = c("red", "skyblue", "hotpink", "forestgreen", "orange"))  
...
spiny_counts |> 
    group_by(mpa) |> 
ggplot() +
    geom_violin(aes(x = mpa, y = counts, color = mpa),
                draw_quantiles = c(0.25, 0.5, 0.75)) +
    labs(x = "MPA",
         y = "Count",
         title = "California Spiny Lobster (*Panulirus Interruptus*) Count by MPA Status") +
    scale_color_manual(values = c("#D77A61", "#5F7470"))
...
spiny_counts |> 
    ...
    geom_vline(xintercept =  mean(mean_size, na.rm = TRUE)) +
