---
title: "California Spiny Lobster (*Panulirus Interruptus*): Assessing the Impact of Marine Protected Areas (MPAs) at 5 Reef Sites in Santa Barbara County" 
date: "1/17/26"
massage: false
warning: false
---
...
ggplot() +
    ...
    scale_fill_manual(values = c("#D77A61", 
                                 "#5F7470"),
                      guide = "none") +
    theme_minimal()
...
ggplot() +
    ...
    labs(x = "Count",
         ...
         subtitle = "Boxplots indicate quartile ranges of data observations") +      
...
spiny_counts |> 
    ...
    labs(y = "Count",
         ...
         subtitle = "Vertical line signifies mean for the group based on color") +
    theme_minimal()
...
As described above, the values for the Poisson regression model and negative binomial model are the same. The average number of lobsters in non-MPA sites is equal across the three models. However, the Poisson regression model and the negative binomial model have slightly lower average increase of 5.23 lobsters for MPA sites compared to non-MPA sites. All of these models provide relatively similar lobster counts once translated into the response space. 

All of the models have significant p-values for the intercept with p-values < 0.0001. Only the Poisson regression model has a significance for the treat covariate.
...
SUTVA cannot be assumed in the context of MPA treatment sites. As discussed above, there is spillover in the lobster counts between the MPA and non-MPA sites meaning that there is a violation of the "no inference" assumption. The second SUTVA assumption, "no hidden variation" is met. All of the MPA sites have the same treatment. In theory no differences exist between the treatment of all of the MPA sites. 
